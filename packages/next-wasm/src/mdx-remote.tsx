...
function sanitizeScope(scope: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [key, value] of Object.entries(scope)) {
    ...
    if (typeof value === 'function') {
      console.warn(`[bunny-next-wasm] Ignoring function in scope: ${key}`);
      continue;
    }

    // Filter out prototype pollution vectors
    if (key === '__proto__' || key === 'constructor' || key === 'prototype') {
      console.warn(`[bunny-next-wasm] Ignoring dangerous property: ${key}`);
      continue;
    }
...
`;
      ...
      console.error('[bunny-next-wasm] Failed to hydrate MDX:', error);
