...
//! MDX source
//!    ↓
//! bunny-mdx (compile MDX → JSX) [always available]
//!    ↓
//! fob-core (bundle JSX + imports → single .js) [optional bundler feature]
//!    ↓
//! Executable JavaScript string
//! ```

// Re-export bunny-mdx types and functions (always available)
pub use bunny_mdx::{
    compile, FrontmatterData, FrontmatterFormat, MdxCompileOptions, MdxCompileResult, MdxError,
};

// Convenience wrapper for compile function
use anyhow::Result;
use bunny_mdx::MdxCompileOptions as Options;

/// Compile MDX to JSX (convenience wrapper)
///
/// This is a simple wrapper around `bunny_mdx::compile()` that provides
/// a convenient API for basic MDX compilation without bundling.
///
/// # Example
///
/// ```rust
/// use bunny::{compile_mdx, MdxCompileOptions};
///
...
pub fn compile_mdx(
    source: &str,
    options: Options,
) -> Result<bunny_mdx::MdxCompileResult, Box<bunny_mdx::MdxError>> {
